...
public class MinigameHandler : MonoBehaviour
{
    ...
    public void Success()
    {
        ...
        if (currentSuccessCount == requiredSuccessCount && completed != true)
        {
            // MinigameOptionUpdate();
    ...
    public void NotSuccess()
    {
        ...
        if (dataManager.debugOnInfo == true || dataManager.debugOnInfoPriority == true)
        {
            Debug.Log("Success count decreased for current Minigame= " + currentSuccessCount + " Target: " + requiredSuccessCount);
        }
    }
    public void Failure()
    {
        ...
        if (currentFailureCount == requiredFailureCount && completed != true)
        {
            // MinigameOptionUpdate();
    ...
    private void DisplayLeaderboard(string result)
    {
        ...
        else if (result == "failure" && completed != true)
        {
            ...
            if (dataManager.debugOnInfo == true || dataManager.debugOnInfoPriority == true)
            {
                Debug.Log("Player failed. Previous leaderboard position: " + dataManager.previousPosition + ". New Position: " + dataManager.currentPosition);
            }
            
        }
    }

    //NOTE: I've moved MinigameOptionUpdate and MinigameOptionReset into Scene Manager Custom.
        // 1. Because they're not really related to transitioning between minigames
        // 2. Because really only Scene Manager Custom should care about Scenes.
}
